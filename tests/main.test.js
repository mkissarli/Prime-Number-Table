//import { is_prime, sieve_n, make_multi_table, prepare_for_output, is_array_of_chars } from '../dist/main.js';
const main = require('../src/main.js');
const is_prime = main.is_prime;
const sieve_n = main.sieve_n;
const make_row_table = main.make_row_table;
const is_array_of_chars = main.is_array_of_chars;
...
describe('sieve_under_n', () => {
    ...
    test('sieve of n != int returns an error', () => {
        expect(sieve_n("p")).toStrictEqual(new TypeError("n must be an integer."));
        expect(sieve_n(10.22)).toStrictEqual(new TypeError("n must be an integer."));
        expect(sieve_n([0])).toStrictEqual(new TypeError("n must be an integer."));
    });
    test('sieve of n <= 0 returns an error', () => {
        expect(sieve_n(0)).toStrictEqual(new TypeError("n must be 1 or greater."));
        expect(sieve_n(-1)).toStrictEqual(new TypeError("n must be 1 or greater."));
        expect(sieve_n(-100)).toStrictEqual(new TypeError("n must be 1 or greater."));
    });
    test('sieve of first 1 returns an array of length 1', () => {
        expect(sieve_n(1)).toStrictEqual(primes.slice(0, 1))
    });
    test('sieve of first 3 returns an array of length 3', () => {
        expect(sieve_n(3)).toStrictEqual(primes.slice(0, 3))
    });
    
    test('sieve of first 8 returns an array of length 8', () => {
        expect(sieve_n(8)).toStrictEqual(primes.slice(0, 8))
    });
    test('sieve of first 15 returns an array of length 15', () => {
        expect(sieve_n(15)).toStrictEqual(primes.slice(0, 15))
    });
    test('sieve of first 24 returns an array of length 24', () => {
        expect(sieve_n(24)).toStrictEqual(primes.slice(0, 24))
    });

    test('sieve of first 20000 returns an array of length 20000', () => {
        expect(sieve_n(20000)).toStrictEqual(primes.slice(0, 20000))
    });
});
...
describe('make_row_table', () => {
    test('throw error if passed a non-integer for a', () => {
        expect(make_row_table([], [1,1,1])).toStrictEqual(new TypeError("ensure a is a integer."));
        expect(make_row_table("k", [1,1,1])).toStrictEqual(new TypeError("ensure a is a integer."));
        expect(make_row_table(null, [1,1,1])).toStrictEqual(new TypeError("ensure a is a integer."));
    });
    test('throw error if passed not passed a list of integers for B', () => {
        expect(make_row_table(1, ["c"])).toStrictEqual(new TypeError("ensure B is an array of integers."));
        expect(make_row_table(1, "c")).toStrictEqual(new TypeError("ensure B is an array of integers."));
        expect(make_row_table(1, 1)).toStrictEqual(new TypeError("ensure B is an array of integers."));
        expect(make_row_table(1, null)).toStrictEqual(new TypeError("ensure B is an array of integers."));
    });
    test('check correct values', () => {
        expect(make_row_table(1, [1, 1])).toStrictEqual([1, 1]);
        expect(make_row_table(2, [3, 2, 3])).toStrictEqual([6, 4, 6]);
        expect(make_row_table(1, [1])).toStrictEqual([1]);
        expect(make_row_table(4, [1, 2, 3])).toStrictEqual([4, 8, 12]);
        expect(make_row_table(1, [])).toStrictEqual([]);
    });
});
...
describe('prepare_for_output', () => {
    ...
    test('prepare_for_output passed [[1], [1] and [1]', () => {
        expect(prepare_for_output([[1]], [1], [1])).toStrictEqual(
            [["\\", 1],
             [1,    1]]
        )
    });
    test('prepare_for_output passed [[1, 2], [2, 4]], [1, 2] and [1, 2]', () => {
        expect(prepare_for_output([[1, 2], [2, 4]], [1, 2], [1, 2])).toStrictEqual(
            [["\\", 1, 2],
             [1,    1, 2],
             [2,    2, 4]]
        )
